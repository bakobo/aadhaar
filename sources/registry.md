...
## 8. Extraction flags — four failures in one corpus, all non-empty and plausible
...
**(d) The extractor's own cleaning, which splits lines it used to join.** Recorded 2026-09-16, re-harvesting against `id-law-kit` a0bad96. `lawcorpus.pdf` now compiles every drafting tradition into one opener pattern by default, and the Indonesian entry's `[a-z0-9]{1,3}\.[ \t]` matches ordinary English wraps — `v.`, `law.`, `it.`, `to.`, `up.`, `etc.` The rejoiner therefore refuses **83 joins across the three judgments** that it used to make (45 in `PUTTASWAMY-2018-SCR`, 27 in `PUTTASWAMY-2018`, 11 in `PUTTASWAMY-2017`, 0 in the one half-Hindi instrument), and `lawcorpus.cite.Corpus.grep` reports lines, so a phrase spanning a refused join cannot be found. Measured: `Mahant Moti Das v. S.P. Sahi` and `Maharashtra University of Health Sciences v. Satchikitsa` are in these judgments and both go from one line-hit to **zero**; `separation of powers and rule of law` goes 3 → 2 and `correctness or lack of it` 4 → 3, while every left half still hits. Re-extracting the SCR judgment with `structural_pattern("common-law")` restores all 45 joins and returns the file from 4,565 lines to 4,520, which isolates the cause. Not worked around here: `extract` takes no `traditions=` argument, so narrowing the pattern means rebinding another package's private module global. Reported to the kit; ~2j4h.

Alongside it, `FURNITURE_LINES = 8` now reaches deeper than the old window, and in a law report the thing at that depth is a footnote marker. `PUTTASWAMY-2017` lost **192 bare numbers** — sequential, increasing down the page, indistinguishable from a page number to `_counts_up` — and `PUTTASWAMY-2018` lost two footnote *contents* that match `_PAGE_NUMBER` in full: `page 420` in footnote 83 and `page 5` in footnote 159. No prose moved in any of the four changed texts: over all four, every token lost is a bare number or the word `page`, and none gained a token. ~45l7
